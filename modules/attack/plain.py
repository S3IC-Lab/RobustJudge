from modules.attack.base_attack import JudgeAttacker
from utils.prompt import (
    get_translation_init_prompt, get_code_init_prompt, get_math_init_prompt,
    get_translation_pure_prompt, get_knowledge_init_prompt, extract_text,
    get_summarization_init_prompt
)
from modules.data import modify_model_generated

class TranslationPlain(JudgeAttacker):
    def __init__(self,benchmark_evaluator,attack_instance, target_instance,args):
        """
        Initialize the plain mode which means no attack actions is taken.
        """
        # super().__init__(model, dataset)
        self.result = None
        self.evaluator = benchmark_evaluator
        self.attack_instance = attack_instance
        self.target_instance = target_instance
        self.args = args
        self.is_system_should_optimize = False
        self.custom_injections = self.args.attack_custom_injections or {}
     
    def get_init_prompt(self, item):
        return get_translation_pure_prompt(self.args)

    def get_prompt_with_source(self, source):
        return get_translation_init_prompt(source)
      
    def att_model_generate(self, prompt, item):
        prompt = prompt.replace("##SOURCE_LAN##", self.args.source)\
                .replace("##TARGET_LAN##", self.args.target)
        if self.model_generated != None:
            return self.model_generated
        content = self.attack_instance.model_generate(prompt)
        output = extract_text(content)
        modify_model_generated(self.cache_data_path, self.cache_data_index, output)
        return output

    def get_bench_score(self, item, reference, candidate):
        return self.evaluator.meta_evaluate(item, reference, candidate)
    
    def run(self, item):
        pass

class MathPlain(JudgeAttacker):
    def __init__(self,benchmark_evaluator,attack_instance, target_instance,args):
        """
        Initialize the plain mode which means no attack actions is taken.
        """
        # super().__init__(model, dataset)
        self.result = None
        self.evaluator = benchmark_evaluator
        self.attack_instance = attack_instance
        self.target_instance = target_instance
        self.args = args
        self.is_system_should_optimize = False
        self.custom_injections = self.args.attack_custom_injections or {}
    
    def get_init_prompt(self, item):
        source = item['source']
        return get_math_init_prompt(source)

    def get_prompt_with_source(self, source):
        return get_math_init_prompt(source)
      
    def att_model_generate(self, prompt, item):
        if self.model_generated != None:
            return self.model_generated
        content = self.attack_instance.model_generate(prompt)
        output = extract_text(content)
        modify_model_generated(self.cache_data_path, self.cache_data_index, output)
        return output
    
    def get_bench_score(self, item, reference, candidate):
        return self.evaluator.meta_evaluate(item, reference, candidate)
    
    def run(self, item):
        pass

class KnowledgePlain(JudgeAttacker):
    def __init__(self,benchmark_evaluator,attack_instance, target_instance,args):
        """
        Initialize the plain mode which means no attack actions is taken.
        """
        # super().__init__(model, dataset)
        self.result = None
        self.evaluator = benchmark_evaluator
        self.attack_instance = attack_instance
        self.target_instance = target_instance
        self.args = args
        self.is_system_should_optimize = False
        self.custom_injections = self.args.attack_custom_injections or {}
    
    def get_init_prompt(self, item):
        source = item['source']
        return get_knowledge_init_prompt(source)

    def get_prompt_with_source(self, source):
        return get_knowledge_init_prompt(source)
      
    def att_model_generate(self, prompt, item):
        if self.model_generated != None:
            return self.model_generated
        content = self.attack_instance.model_generate(prompt)
        output = extract_text(content)
        modify_model_generated(self.cache_data_path, self.cache_data_index, output)
        return output
    
    def get_bench_score(self, item, reference, candidate):
        return self.evaluator.meta_evaluate(item, reference, candidate)
    
    def run(self, item):
        pass 

class CodePlain(JudgeAttacker):
    def __init__(self,benchmark_evaluator,attack_instance, target_instance, args):
        """
        Initialize the plain mode which means no attack actions is taken.
        """
        # super().__init__(model, dataset)
        self.result = None
        self.evaluator = benchmark_evaluator
        self.attack_instance = attack_instance
        self.target_instance = target_instance
        self.args = args
        self.is_system_should_optimize = False
        self.custom_injections = self.args.attack_custom_injections or {}
    
    def get_init_prompt(self, item):
        if self.args.task == "code_translation":
            self.args.source = item['source_lang']
            self.args.target = item['target_lang']
        source = item['source']
        return get_code_init_prompt(source, self.args)

    def get_prompt_with_source(self, source):
        return get_code_init_prompt(source, self.args)
      
    def att_model_generate(self, prompt, item):
        if self.model_generated != None:
            return self.model_generated
        content = self.attack_instance.model_generate(prompt)
        output = extract_text(content)
        modify_model_generated(self.cache_data_path, self.cache_data_index, output)
        return output
    
    def get_bench_score(self, item, reference, candidate):
        return self.evaluator.meta_evaluate(item, reference, candidate)
    
    def run(self, item):
        pass   


class SummarizationPlain(JudgeAttacker):
    def __init__(self,benchmark_evaluator,attack_instance, target_instance,args):
        """
        Initialize the plain mode which means no attack actions is taken.
        """
        # super().__init__(model, dataset)
        self.result = None
        self.evaluator = benchmark_evaluator
        self.attack_instance = attack_instance
        self.target_instance = target_instance
        self.args = args
        self.is_system_should_optimize = False
        self.custom_injections = self.args.attack_custom_injections or {}
    
    def get_init_prompt(self, item):
        source = item['source']
        return get_summarization_init_prompt(source)

    def get_prompt_with_source(self, source):
        return get_summarization_init_prompt(source)
      
    def att_model_generate(self, prompt, item):
        if self.model_generated != None:
            return self.model_generated
        content = self.attack_instance.model_generate(prompt)
        output = extract_text(content)
        modify_model_generated(self.cache_data_path, self.cache_data_index, output)
        return output
    
    def get_bench_score(self, item, reference, candidate):
        return self.evaluator.meta_evaluate(item, reference, candidate)
    
    def run(self, item):
        pass 