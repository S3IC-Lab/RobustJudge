...
class RetokenizationDefense(JudgeDefender):
    def __init__(self, attack_instance, defense_instance, args):
        super().__init__(attack_instance, defense_instance, args)
        merge_table = load_subword_nmt_table('dataset/for_defense/subword_nmt.voc')
        self.retokenizer = BpeOnlineTokenizer(bpe_dropout_rate=0.1, merge_table=merge_table)

    def gen_retokenzied_data(self, data_prompt):
        for _ in range(10):
            try:
                return self.retokenizer(data_prompt, sentinels=['', '</w>'], regime='end', bpe_symbol='@@')
            except:
                continue
        logger.info(f'WARNING: unable to retokenize this sentence')
        return data_prompt

    def make_retokenized_prompt(self, item):
        args = self.args
        retokenized_data = self.gen_retokenzied_data(item['malicious answer'])
        logger.info(f"retokenized_data:{retokenized_data}")
        if 'id' not in item:
            return retokenized_data
        if args.judge == 'score':
            self.score_prompt = get_score_prompt(item['source'], retokenized_data, args)
        else:
            # pairwise judge
            self.pairwise_prompt_A = get_pair_prompt(item['source'], retokenized_data, item['target'], args)
            self.pairwise_prompt_B = get_pair_prompt(item['source'], item['target'], retokenized_data, args)
        return retokenized_data
    ...
    def run(self):
        ...
        for i, item in enumerate(data):
            ...
            result[i]['defense_retokenized_content'] = self.make_retokenized_prompt(item)
...
# https://github.com/liu00222/Open-Prompt-Injection/blob/main/OpenPromptInjection/apps/bpe.py
import heapq
import numpy as np
